...
window.buildBranding = function(image, boxes) {
  const size=256,atlas=document.createElement('canvas');atlas.width=size*3;atlas.height=size;
  ...
  boxes.forEach((box,k)=>{
    ...
    for(let y=0;y<size;y++)for(let x=0;x<size;x++){
      ...
      if(k===2)mask[i]=((x/size-.5)/.47)**2+((y/size-.5)/.47)**2<1?1:0;
      else {
        const lightInk=r>115&&g>115&&b-r<85;
        const redInk=k===0&&r>110&&g<110&&b<125;
        const navyInk=k===0&&r<65&&g<95&&b<135;
    ...
    for(let i=0;i<mask.length;i++){
      ...
      if(k<2&&!mask[i]){data.data[i*4]=247;data.data[i*4+1]=248;data.data[i*4+2]=250;}
    }
    ...
    for(let y=0;y<h;y++)for(let x=0;x<w;x++){
      ...
      if(outside[iy*size+ix]<12)erase[(top+y)*source.width+left+x]=1;
    }
  });
